...
func _on_interstitial_state_changed(args):
	...
	elif args[0] == "closed" or args[0] == "failed":
		GlovalVAR.GameContinue=true
		AudioServer.set_bus_volume_db(0,0)
		GlovalVAR.emit_signal('InterstitialAdEnded')
...
func _on_rewarded_state_changed(args):
	...
	if args[0] == "opened":
		...
		GlovalVAR.GameContinue=true
		AudioServer.set_bus_volume_db(0,0)
		GlovalVAR.emit_signal('RewardedAdEnded',false)
	elif args[0]=='rewarded':
		AudioServer.set_bus_volume_db(0,0)
		if int(GlovalVAR.rewordState) == 1:
			GlovalVAR.DATA["Money"] += 1
		elif int(GlovalVAR.rewordState) == 2:
			GlovalVAR.DATA["Gold"] += 0.5

		GlovalVAR.emit_signal('RewardedAdEnded',true)
	elif args[0]=='failed':
		GlovalVAR.GameContinue=true
		AudioServer.set_bus_volume_db(0,0)
		GlovalVAR.emit_signal('RewardedAdEnded',false)
